...
exports.parseMatchesForStageAndGroups = (callback, stage, group) => {
    let subStr1 = "";
    let subStr2 = "";
    let resultArr = [];
...
exports.parseCurrentMatches = (callback) => {
    let sqlTransaction = "SELECT TABLE_MATCHES.matchID, TABLE_MATCHES.stageID as STAGE, (SELECT TABLE_TEAMS.teamname FROM TABLE_TEAMS WHERE TABLE_MATCHES.teamoneID = TABLE_TEAMS.teamID) TEAM1, (SELECT TABLE_TEAMS.teamname from TABLE_TEAMS WHERE TABLE_MATCHES.teamtwoID = TABLE_TEAMS.teamID) TEAM2 FROM TABLE_MATCHES HAVING TABLE_MATCHES.stageID = " + CURRENT_STAGE + " AND (team1 is not null and team2 is not null)";
    let resultArr = [];
...
exports.addPlayer = (callback, name) => {
    console.log("addPlayer")
    let id = (getPlayerID(name), (result) => {
        console.log("id callback: " + result);
        return result;
    });
    let sqlTransaction;
    ...
    sqlTransaction = "INSERT INTO TABLE_PLAYERS (playername, playerscore) values (" + name + ", 0.0)";

    con.connect((err) => {
        con.query(sqlTransaction, (err, result) => {
            if (err) return err;
            return {
                "id": (getPlayerID(name), (result) => {
                    return result;
                })
            };
        });
...
function getPlayerID(name) {
    con.connect((err) => {
        con.query("SELECT TABLE_PLAYERS.playerID from TABLE_PLAYERS WHERE TABLE_PLAYERS.playerName = '" + name + "'", (err, result) => {
            if (err) {
                console.log(err);
                return -1;
            }
